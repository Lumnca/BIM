通过Three相关知识可以再加一个零件模型，这里零件模型为STL格式
...
这样就可以我们点哪里零件就在哪里。但是这里有个问题，就是零件会以初始形态固定朝向显示在模型中，不管在哪里，所以这里就需要我们判断零件在模型上的时候能像视角对正那样可以正视角显示。

参考视角对齐，我们就知道在定位的时候应该让零件朝向点击点法向量，这样就能保持我们的零件每次是与切面平行的。同时在一开始的时候没有设置安装面，所以要在一开始就要确认正确的安装面：
...
if(stlMesh){
	...
	//朝向法向量
    stlMesh.lookAt(n);
    //所谓安装面就是模型相对与安装时候需要旋转的角度这个值需要自己在实际中设置
